...
def finetune(reading_params_path, finetune_corpus_path, pretrain_dataset, block_size, model):
    ...
    if reading_params_path is None:
        ...
        model.load_state_dict(torch.load(reading_params_path), strict=False)
...
def train(model, writing_params_path, trainer_obj):
    ...
    setattr(trainer_obj, 'model', model)
    setattr(trainer_obj, 'config.ckpt_path', writing_params_path)
    trainer_obj.train()
